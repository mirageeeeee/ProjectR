...
namespace RPGMods.Utils
{
    public static class Helper
    {
        ...
        public static Entity AddItemToInventoryWithoutChat(EntityManager entityManager, Entity sender, PrefabGUID guid, int amount)
        {
            unsafe
            {
                var gameData = Plugin.Server.GetExistingSystem<GameDataSystem>();
                ...
                var boxedBytePtr = IntPtr.Subtract(bytePtr, 0x10);
                var hack = new Il2CppSystem.Nullable<int>(boxedBytePtr);
                var hasAdded = InventoryUtilitiesServer.TryAddItem(entityManager, gameData.ItemHashLookupMap, sender, guid, amount, out _, out Entity e, default, hack);
                return e;
            }
        }
